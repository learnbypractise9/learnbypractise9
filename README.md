# Hi there, I'm Ashwin Bhandurge 👋

**Senior Data & ML  Engineer** | 14+ years across Regulatory Banking, Credit Risk, Cloud Modernization & Implmenting AI/ML solutions

📍 Brussels, Belgium &nbsp;|&nbsp; 📧 ashwin.bhandurge@gmail.com

---

## 🧑‍💼 About Me

I'm a Senior ML & Data Engineer at **Capgemini Belgium** working with **Global Credit Data (GCD)**, specializing in regulatory data platforms, **large-scale SAS-to-Cloud migrations** and bringing optimization and efficiency through **AI & ML solutions** for the client. I bridge the gap between business, regulatory, and engineering teams to deliver compliant, scalable data solutions for global tier-1 banks.

With a unique blend of **engineering**, **academic teaching**, **banking/risk management** and **AI/ML** experience, I bring both technical depth and the ability to communicate complex ideas clearly to diverse stakeholders.

Recently, architecting a RAG based Gen AI solution with proprietary & local LLMs and mentoring a team of developers in implementing the solution for the client.   
---

## 🛠️ Tech Stack

**Data Engineering & ETL**
![SAS](https://img.shields.io/badge/SAS-blue?style=flat-square) ![Python](https://img.shields.io/badge/Python-3776AB?style=flat-square&logo=python&logoColor=white) ![PySpark](https://img.shields.io/badge/PySpark-E25A1C?style=flat-square&logo=apachespark&logoColor=white) ![SQL](https://img.shields.io/badge/SQL-4479A1?style=flat-square&logo=postgresql&logoColor=white)

**Cloud & Platforms**
![Azure](https://img.shields.io/badge/Azure-0078D4?style=flat-square&logo=microsoftazure&logoColor=white) ![Microsoft Fabric](https://img.shields.io/badge/Microsoft%20Fabric-742774?style=flat-square&logo=microsoft&logoColor=white) ![Azure DevOps](https://img.shields.io/badge/Azure%20DevOps-0078D7?style=flat-square&logo=azuredevops&logoColor=white)

**ML & AI**
![ML](https://img.shields.io/badge/Machine%20Learning-FF6F00?style=flat-square) ![NLP](https://img.shields.io/badge/NLP-4B8BBE?style=flat-square) ![Deep Learning](https://img.shields.io/badge/Deep%20Learning-FF0000?style=flat-square)

**Tools**
![GitHub](https://img.shields.io/badge/GitHub-181717?style=flat-square&logo=github&logoColor=white) ![GitHub Copilot](https://img.shields.io/badge/GitHub%20Copilot-000000?style=flat-square&logo=github&logoColor=white) ![Tableau](https://img.shields.io/badge/Tableau-E97627?style=flat-square&logo=tableau&logoColor=white) ![PowerApps](https://img.shields.io/badge/PowerApps-742774?style=flat-square&logo=powerapps&logoColor=white)

---

## 💼 Experience Highlights

### 🏦 Capgemini Belgium — Senior Consultant *(Apr 2019 – Present)*
- Architecting a PDF data extraction and standardization **Gen AI solution**  
- **Lead** multi-year SAS-to-Cloud migration for GCD's LGD, PD, Benchmarking & ICC platforms
- Built **PySpark ETL notebooks**, Python validation engines, and Azure Functions/Batch pipelines on **Microsoft Fabric**
- Designed **data governance frameworks**: LEI submission flows, anonymized ID generation, SCD-based historical data
- Implemented **NLP fuzzy name-matching** using Word Vectorization, TF-IDF & Cosine Similarity
- Represented Capgemini at **GCD conferences** in London, Frankfurt & Zurich; contributed to GenAI & ESG discussions

### 🏛️ Central Bank of India — Manager, Risk Management *(Jun 2012 – Feb 2014)*
- Implemented **BASEL II credit risk models** (standardized & IRB approaches) on SAS
- Served as the central liaison between Deloitte's model development team, TCS IT, and bank management
- Generated regulatory reports for CRAR calculation; automated testing with Unix shell scripts

### 🎓 University of Mumbai — Assistant Professor *(Jun 2014 – May 2018)*
- Taught **Financial Markets, Python, Debt/Equity Markets, Foreign Exchange, IT in Banking**
- Developed *TARS* (Teachers' Assessment Rating System) using Excel Macros
- Founded and edited the student monthly newsletter *Chronicle 30*

---

## 🎓 Education

| Degree | Institution | Year | Specialization |
|--------|------------|------|----------------|
| MBA | Symbiosis International University | 2012 | Banking Management |
| B.E. | University of Mumbai | 2006 | Computer Engineering |
| Diploma | Maharashtra Board | 2003 | Computer Technology |

---

## 📜 Certifications

- 🤖 **Machine Learning A-Z** — Hands-On Python & R in Data Science
- 🔄 **PSM-I** — Professional Scrum Master (Agile)
- 📊 **SAS Data Integration Studio** — Fast Track Classroom Training
- 🇫🇷 French Language — Level 1.2 (C.V.O. Brussels)
- 🏫 **MH-SET** — Maharashtra State Eligibility Test for Assistant Professor

---

## 🌍 Languages

🇬🇧 English &nbsp;|&nbsp; 🇫🇷 French &nbsp;|&nbsp; 🇮🇳 Hindi

---

## 🎯 Interests

♟️ Chess &nbsp;|&nbsp; 📈 Stock Trading &nbsp;|&nbsp; 📚 Reading (Fiction) &nbsp;|&nbsp; 👨‍🏫 Teaching

---

*"Bridging business, regulatory, and engineering worlds — one data pipeline at a time."*
